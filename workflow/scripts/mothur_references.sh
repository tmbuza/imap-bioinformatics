...
mothur "#set.logfile(name="${LOGS}"/degapping_silva.logfile);
    degap.seqs(fasta=data/mothur/references/silva.seed.align, inputdir="${OUTDIR}", outputdir="${OUTDIR}");
    degap.seqs(fasta=data/mothur/references/silva.v4.align, inputdir="${OUTDIR}", outputdir="${OUTDIR}")"