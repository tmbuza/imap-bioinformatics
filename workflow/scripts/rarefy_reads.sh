...
SAMPLESHARED="data/mothur/final/otu_analysis/sample.final.shared" # Shared file to be counted


OUTDIR=$(echo "${SAMPLESHARED}" | sed 's:\(.*/\).*:\1:')
...
TMP_SHARED=$(echo "${SAMPLESHARED}" | sed 's:'"${OUTDIR}"':'"${TMP}"':')
