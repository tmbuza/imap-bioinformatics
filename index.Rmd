...
```{r}
library(tidyverse, suppressPackageStartupMessages())
sraruntable <- read_delim("data/mothur/metadata/SraRunTable.txt", show_col_types = FALSE)
...
shared <- read_tsv("data/mothur/process/final.shared", show_col_types = F)
head(shared)
```
...
```{r}
head(read_tsv("data/mothur/process/final.cons.taxonomy", show_col_types = F))
```
...
taxonomy <- read_tsv("data/mothur/process/final.cons.taxonomy", show_col_types = F) %>%
  rename_all(tolower) %>%
  select(otu, taxonomy) %>%
