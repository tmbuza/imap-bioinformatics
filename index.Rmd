...
shared <- read_tsv("data/process/final.opti_mcc.shared", show_col_types = F)
head(shared)
```
...
```{r}
head(read_tsv("data/process/final.opti_mcc.0.03.cons.taxonomy", show_col_types = F))
```
...
taxonomy <- read_tsv("data/process/final.opti_mcc.0.03.cons.taxonomy", show_col_types = F) %>%
  rename_all(tolower) %>%
  select(otu, taxonomy) %>%
